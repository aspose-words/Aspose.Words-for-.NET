...
namespace ApiExamples
{
    ...
    public class ExVariableCollection : ApiExampleBase
    {
        ...
        public void Primer()
        {
            ...
            Assert.IsTrue(variables.Contains("City"));
            Assert.IsTrue(variables.Any(v => v.Value == "London"));
            ...
            using (IEnumerator<KeyValuePair<string, string>> enumerator = doc.Variables.GetEnumerator())
                while (enumerator.MoveNext())
                    Console.WriteLine(string.Format("Name: {0}, Value: {1}", enumerator.Current.Key, enumerator.Current.Value));

            // Below are three ways of removing document variables from a collection.
            // 1 -  By name:
            variables.Remove("City");

            Assert.IsFalse(variables.Contains("City"));

            // 2 -  By index:
            variables.RemoveAt(1);

            Assert.IsFalse(variables.Contains("Home address"));
